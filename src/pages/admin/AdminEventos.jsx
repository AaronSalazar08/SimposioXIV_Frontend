...
export default function AdminEventos() {
  ...
  const openCrear = () => { setForm(EMPTY_FORM); setError(''); setModal({ open: true, evento: null }) }
  const openEditar = (ev) => {
    setForm({
      titulo: ev.titulo,
      ...
      ponente_id: String(ev.ponente?.id ?? ev.ponente_id ?? ''),
  ...
  const saveMutation = useMutation({
    mutationFn: (data) => {
      const payload = {
        ...
        ponente_id: data.ponente_id ? Number(data.ponente_id) : null,
      }
      return modal.evento
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                {['Tipo', 'Título', 'Horario', 'Cupos', 'Estado', ''].map((h) => (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">
              Ponente <span className="text-gray-400 font-normal">(opcional)</span>
            </label>
            <select className={SELECT_CLASS} value={form.ponente_id} onChange={set('ponente_id')}>
              <option value="">Sin ponente</option>
              {ponentes.map((p) => <option key={p.id} value={p.id}>{p.nombre} {p.apellidos}</option>)}
            </select>
          </div>
          {areas.length > 0 && (
            <div>
