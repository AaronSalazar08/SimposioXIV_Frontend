...
function ChapterTag({ n, label, color = '#21BBEF' }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 12, fontFamily: "'Space Mono', monospace", fontSize: 'clamp(14px,1.3vw,16px)', letterSpacing: '0.18em', textTransform: 'uppercase', color }}>
      <span>({n})</span>
    ...
    <section style={{ position: 'relative', minHeight: '100vh', overflow: 'hidden', background: 'linear-gradient(145deg, #010810 0%, #001020 35%, #001a38 65%, #002650 100%)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ position: 'relative', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', maxWidth: 1560, width: '100%', margin: '0 auto', padding: '80px clamp(20px,4vw,56px) 0' }}>
        ...
        <h1 style={{ margin: 0, fontFamily: "'Space Grotesk', sans-serif", fontWeight: 600, fontSize: 'clamp(2.2rem, 8.6vw, 8.6rem)', lineHeight: 0.92, letterSpacing: '-0.045em', color: '#fff' }}>
          <Lines lines={[
            'El futuro',
            'se construye',
            <span key="c">en <span className="ed" style={{ color: '#21BBEF', fontSize: '1.06em' }}>Guanacaste.</span></span>,
          ]} />
        </h1>
        ...
        <Fade delay={0.62} style={{ marginTop: 'clamp(28px,4vh,42px)' }}>
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
            <Link to="/inscripciones" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, height: 58, padding: '0 30px', fontFamily: "'Space Grotesk', sans-serif", fontWeight: 600, fontSize: 'clamp(0.95rem,1.2vw,1.1rem)', borderRadius: 9999, background: '#21BBEF', color: '#05070E', border: '1px solid transparent', textDecoration: 'none', transition: `all 200ms ${EASE_OUT}`, boxShadow: '0 0 32px rgba(33,187,239,0.35)' }}
              ...
              Reservar mi lugar <span style={{ fontFamily: "'Space Mono', monospace" }}>→</span>
            </Link>
      ...
      <div style={{ position: 'relative', borderTop: '1px solid rgba(255,255,255,0.08)', opacity: fade }}>
        <div style={{ maxWidth: 1560, margin: '0 auto', padding: '20px clamp(20px,4vw,56px)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
          <span style={{ fontFamily: "'Space Mono', monospace", fontSize: 'clamp(13px,1.3vw,16px)', letterSpacing: '0.08em', color: '#fff' }}>SIMPOSIO DE INFORMÁTICA EMPRESARIAL <span style={{ color: 'rgba(255,255,255,0.4)' }}>· UCR · SG-CIE</span></span>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 10, fontFamily: "'Space Mono', monospace", fontSize: 15, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 24, marginBottom: 'clamp(40px,7vh,72px)' }}>
          <div>
            ...
            <h2 style={{ margin: 0, fontFamily: "'Space Grotesk', sans-serif", fontWeight: 600, fontSize: 'clamp(2.2rem,6vw,4.8rem)', lineHeight: 0.96, letterSpacing: '-0.04em', color: '#05070E' }}>
              <Lines lines={['Seis frentes', <span key="b">para pensar el <span className="ed" style={{ color: '#005DA4' }}>futuro.</span></span>]} />
            </h2>
          </div>
              ...
              <Fade key={tk.n} delay={i * 0.05}>
                <Link to="/tematicas" style={{ textDecoration: 'none' }}>
                  <div onMouseEnter={() => setHover(i)}
                    ...
                    <span style={{ fontFamily: "'Space Mono', monospace", fontSize: 'clamp(14px,1.3vw,17px)', color: on ? tk.color : 'rgba(0,0,0,0.25)', width: 30, transition: 'color 0.3s' }}>{tk.n}</span>
  ...
  const panels = DAYS.map((d) => (
    <article key={d.n} style={{ width: '100vw', minHeight: stack ? 'auto' : '100vh', flex: 'none', background: d.bg, color: d.fg, position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center' }}>
      ...
      <div style={{ position: 'relative', maxWidth: 1560, width: '100%', margin: '0 auto', padding: stack ? 'clamp(64px,10vh,100px) clamp(20px,5vw,80px)' : '0 clamp(20px,5vw,80px)', display: 'grid', gridTemplateColumns: stack ? '1fr' : '1fr 1fr', gap: 'clamp(24px,5vw,80px)', alignItems: 'center' }}>
        <div>
          <span style={{ fontFamily: "'Space Mono', monospace", fontSize: 'clamp(13px,1.3vw,16px)', letterSpacing: '0.16em', textTransform: 'uppercase', opacity: 0.75 }}>Día {d.n} · {d.day}</span>
        ...
        <ul style={{ listStyle: 'none', margin: 0, padding: 0, borderTop: `1px solid ${d.fg === '#fff' ? 'rgba(255,255,255,0.16)' : 'rgba(5,7,14,0.18)'}` }}>
          {d.kw.map((k, i) => (
            <li key={i} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: 'clamp(16px,2.4vh,26px) 0', borderBottom: `1px solid ${d.fg === '#fff' ? 'rgba(255,255,255,0.16)' : 'rgba(5,7,14,0.18)'}` }}>
              <span style={{ fontFamily: "'Space Mono', monospace", fontSize: 14, opacity: 0.6, width: 22 }}>{String(i + 1).padStart(2, '0')}</span>
  ...
  return (
    <section ref={wrapRef} style={{ height: '340vh', position: 'relative', background: '#05070E' }}>
      <div style={{ position: 'sticky', top: 0, height: '100vh', overflow: 'hidden' }}>
        ...
        <span style={{ position: 'absolute', top: 'calc(50% - min(31vh,16rem) - 14px)', left: '50%', transform: 'translateX(-50%)', fontFamily: "'Space Mono', monospace", fontSize: 15, letterSpacing: '0.2em', textTransform: 'uppercase', color: idx === 2 ? 'rgba(5,7,14,0.55)' : 'rgba(255,255,255,0.35)', zIndex: 5, pointerEvents: 'none' }}>(03) — Tres días</span>
      </div>
    </section>
  )
}
...
function Venue() {
  ...
  return (
    <section style={{ position: 'relative', overflow: 'hidden', background: '#05070E', minHeight: '100vh', display: 'flex', alignItems: 'flex-end' }}>
      {/* Aerial photo — reemplazar heroImg con la foto aérea real de la sede */}
      ...
      <div style={{ position: 'relative', width: '100%', maxWidth: 1560, margin: '0 auto', padding: 'clamp(48px,10vh,120px) clamp(20px,4vw,56px)' }}>
        <div style={{ marginBottom: 24 }}><ChapterTag n="05" label="La sede" /></div>
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.4fr) minmax(0,1fr)', gap: 'clamp(24px,4vw,64px)', alignItems: 'end' }}>
          <h2 style={{ margin: 0, fontFamily: "'Space Grotesk', sans-serif", fontWeight: 600, fontSize: 'clamp(2.6rem,8vw,7rem)', lineHeight: 0.9, letterSpacing: '-0.045em', color: '#fff' }}>
            <Lines lines={['Liberia,', <span key="b"><span className="ed" style={{ color: '#21BBEF' }}>Guanacaste.</span></span>]} />
          </h2>
          <Fade delay={0.2}>
            <p style={{ margin: 0, fontSize: 'clamp(1rem,1.4vw,1.2rem)', lineHeight: 1.6, color: 'rgba(255,255,255,0.65)' }}>
